...
public:
  ...
  // usblink callbacks
  static void usblink_dirlist_callback_nested(struct usblink_file *file,
                                              bool is_error, void *data);
  static void usblink_dirlist_callback(struct usblink_file *file, bool is_error,
                                       void *data);
...
signals:
  void downloadProgress(int progress); // Called by callback from EmuThread. Use
                                       // Qt::QueuedConnection
  void uploadProgress(int progress);   // Same as above
  void uploadError(int code);          // Error code from calculator

  void wantToAddTreeItem(
      QTreeWidgetItem *item,
      QTreeWidgetItem *parent); // To let it run in the UI thread. Internal
  void wantToReload();          // Same as above

public slots:
  void reloadFilebrowser();
  void showUploadError(int code);
