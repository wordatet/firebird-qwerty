...
enum Address { AddrAll = 0xFF, AddrMe = 0xFE, AddrCalc = 0x01 };
...
static bool writePacket(NNSEMessage *message) {
  ...
  message->csum = 0;
  message->csum = htons(
      compute_checksum(reinterpret_cast<uint8_t *>(message), length) ^ 0xFFFF);

  if (compute_checksum(reinterpret_cast<uint8_t *>(message), length) !=
      0xFFFF) {
  ...
  return usb_cx2_packet_to_calc(1, reinterpret_cast<uint8_t *>(message),
                                length);
}

static uint16_t nextSeqno() { return usblink_cx2_state.seqno++; }
...
static void handlePacket(const NNSEMessage *message,
                         const uint8_t **streamdata = nullptr,
                         int *streamsize = nullptr) {
  ...
  case AddrReqService: {
    ...
    printf("Got request from client %s (product id %c%c)\n", &req->clientID[12],
           req->clientID[10], req->clientID[11]);
#endif
  ...
  case UnknownService: {
    if (ntohs(message->length) != sizeof(NNSEMessage) + 1 ||
        message->data[0] != 0x01)
      goto drop;
...
bool usblink_cx2_handle_packet(const uint8_t *data, uint16_t size) {
  ...
  // The nspire OS adds +1 to packets that are 64-byte aligned to avoid ZLP
  // issues
  ...
  if (compute_checksum(reinterpret_cast<const uint8_t *>(message),
                       completeLength) != 0xFFFF)
    return false;
