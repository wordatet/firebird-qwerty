...
void cx2_backlight_write(uint32_t addr, uint32_t value) {
  ...
  if (cx2_backlight.pwm_period == 0)
    ...
    hdq1w.lcd_contrast =
        255 - (cx2_backlight.pwm_value * 255) / cx2_backlight.pwm_period;
}
...
void dma_cx2_reset() { memset(&dma, 0, sizeof(dma)); }

enum class DMAMemDir { INC = 0, DEC = 1, FIX = 2 };
...
uint32_t dma_cx2_read_word(uint32_t addr) {
  ...
  if (offset >= 0x100 && offset < 0x200) {
    int ch = (offset - 0x100) / 0x20;
    int reg = (offset - 0x100) % 0x20;
    switch (reg) {
    case 0x00:
      return dma.channels[ch].control;
    case 0x04:
      return dma.channels[ch].config;
    case 0x08:
      return dma.channels[ch].src;
    case 0x0C:
      return dma.channels[ch].dest;
    case 0x10:
      return dma.channels[ch].llp;
    case 0x14:
      return dma.channels[ch].len;
    }
  }
...
void dma_cx2_write_word(uint32_t addr, uint32_t value) {
  ...
  if (offset >= 0x100 && offset < 0x200) {
    ...
    switch (reg) {
    case 0x00:
      dma.channels[ch].control = value;
      dma_cx2_update();
      break;
    case 0x04:
      dma.channels[ch].config = value;
      break;
    case 0x08:
      dma.channels[ch].src = value;
      break;
    case 0x0C:
      dma.channels[ch].dest = value;
      break;
    case 0x10:
      dma.channels[ch].llp = value;
      break;
    case 0x14:
      dma.channels[ch].len = value & 0x003fffff;
      break;
    }
    return;
  }
